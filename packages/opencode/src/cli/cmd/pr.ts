...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run waspcode",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found waspcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["waspcode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting waspcode...")
        UI.println()

        // Launch waspcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const waspcodeArgs = sessionId ? ["-s", sessionId] : []
        const waspcodeProcess = spawn("waspcode", waspcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          waspcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`waspcode exited with code ${code}`))
          })
          waspcodeProcess.on("error", reject)
        })
      },
    })
  },
